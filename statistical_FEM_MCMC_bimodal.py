...
# Define pdf of rv zeta
def pi(zeta, alpha=1., beta = 100.0):
...
def getSamples(Y_VALUES, U_VALUES, samples):
    ...
    prior_prev = sp.norm.pdf(w_prev[0], loc_sigma, sigma_sigma)*pi(w_prev[1])
    ...
    while total_samples < samples:
        ...
        prior = sp.norm.pdf(w[0], loc_sigma, sigma_sigma)*pi(w[1])
        if prior == 0:
            None
        else:
...
getSamples(Y_VALUES, U_VALUES, 100000)
